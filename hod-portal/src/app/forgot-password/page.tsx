...
export default function HodForgotPasswordPage() {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatusMessage(`Recovery verification code dispatched to: ${data.email}`);
      if (data.dev_otp) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center px-4 py-12 relative overflow-hidden">
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-96 h-96 bg-emerald-500/10 blur-[130px] rounded-full pointer-events-none" />

      <div className="max-w-md w-full mx-auto space-y-8 relative z-10">
        <div className="text-center space-y-3">
          <div className="inline-flex h-16 w-16 rounded-2xl bg-gradient-to-tr from-emerald-600 to-teal-600 items-center justify-center shadow-xl shadow-emerald-500/20 mb-2">
            <KeyRound className="h-8 w-8 text-white" />
          </div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">HOD Recovery</h1>
          <p className="text-sm text-slate-400">
            {step === 'request' && 'Enter your departmental HOD email to receive a recovery code.'}
            {step === 'verify' && 'Enter the 6-digit code and choose your new HOD password.'}
            {step === 'completed' && 'Your departmental credentials have been updated.'}
          </p>
        </div>
        ...
        >
          ...
          {step === 'request' && (
            <form onSubmit={handleRequestOtp} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full py-3.5 px-4 rounded-xl font-bold text-sm bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 hover:opacity-95 text-white shadow-lg shadow-emerald-600/25 transition flex items-center justify-center space-x-2"
              >
          ...
          {step === 'verify' && (
            <form onSubmit={handleResetPassword} className="space-y-5">
              {devOtp && (
                <div className="p-3 rounded-xl bg-emerald-950/40 border border-emerald-800/40 text-xs text-emerald-300 flex items-center justify-between">
                  <span>Demo Code: <strong className="text-white font-mono text-sm">{devOtp}</strong></span>
                  <span className="text-[10px] text-emerald-400 uppercase">Auto-Filled</span>
                </div>
              )}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="123456"
                  className="w-full text-center tracking-[8px] font-mono text-lg py-3 rounded-xl bg-slate-950 border border-slate-800 text-white focus:outline-none focus:border-emerald-500 transition"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 px-4 rounded-xl font-bold text-sm bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 hover:opacity-95 text-white shadow-lg shadow-emerald-600/25 transition flex items-center justify-center space-x-2"
              >
                <span>{isLoading ? 'Updating...' : 'Update HOD Password'}</span>
            ...
            <div className="text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-400">
                  Your HOD password is now active.
                </p>
              </div>
              <Link
                href="/"
          ...
          <div className="mt-6 pt-5 border-t border-slate-800 text-center text-xs text-slate-500 flex items-center justify-center space-x-2">
            ...
            <span>Strict Department Isolation Security</span>
          </div>
        </motion.div>
      </div>
    </div>
